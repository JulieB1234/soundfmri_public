function timing_files_passive(sub_nb, options)
...
Output_directory = '/Users/julieboyer/Desktop/PHD/SOUNDFMRI 2023/new_scripts_and_tests/PipelinesUniV_Sept24/8mmSmoothing/PASSIVE/TimingFiles'; %sprintf('/Users/julieboyer/Desktop/PHD/SOUNDFMRI 2023/new_scripts_and_tests/Pipelines_tests/PASSIVE/Timing_files');
    ...
    elseif ismember(options.GLMtype,'Pmod')
        ...
        if ismember(options.PModVer,'V1') %[-30 -11.5 -9.5 -7.5 -3] for both vowels
            ...
            while i < ((trials_per_block + 1)+blocknum*trials_per_block)
                ...
                if trials.question_num(i) == 1 && ~isnan(trials.rt1(i)) %quiz
                    onsets{4} = [onsets{4} trials.time_quest(i) - trials.scan_start(i) + trials.rt1(i)];
                ...
                elseif trials.question_num(i) == 2 %RT
                    ...
                    %                 onsets{13} = [onsets{13} trials.time_pause(i) - trials.scan_start(i) + trials.rt2(i)];
                ...
                elseif trials.question_num(i) == 3 && ~isnan(trials.rt1(i)) %mindwandering
                    ...
                    durations{4} = [durations{4} 0];
                elseif trials.question_num(i) == 4 && ~isnan(trials.rt2(i)) %none
                    onsets{4} = [onsets{4} trials.time_pause(i) - trials.scan_start(i) + trials.rt2(i)];
                    durations{4} = [durations{4} 0];
                end
                i = i+1;
            end
